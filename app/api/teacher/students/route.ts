...
import { cleanText, createId, hashPin, jsonError } from "../../_lib";
...
export async function POST(request: Request) {
  const payload = (await request.json().catch(() => null)) as {
    teacherId?: string; classId?: string; seatNo?: string; nickname?: string; pin?: string;
  } | null;
  ...
  if (!teacherId || !classId || !seatNo || !nickname || pin.length < 4) return jsonError("請輸入座號、暱稱與至少 4 碼 PIN。");

  const db = await ensureDb();
  if (!(await ownedActiveClass(db, teacherId, classId))) return jsonError("班級尚未啟用，或你沒有這個班級的管理權。", 403);
  try {
    await db
      .prepare("INSERT INTO students (id, class_id, seat_no, nickname, pin_hash) VALUES (?, ?, ?, ?, ?)")
      .bind(createId("stu"), classId, seatNo, nickname, await hashPin(`${classId}:${seatNo}`, pin))
      .run();
  } catch {
    return jsonError("這個座號已經存在。");
  }
...
export async function PATCH(request: Request) {
  const payload = (await request.json().catch(() => null)) as {
    teacherId?: string; classId?: string; studentId?: string; seatNo?: string; nickname?: string; pin?: string;
  } | null;
  const teacherId = cleanText(payload?.teacherId, 80);
  const classId = cleanText(payload?.classId, 80);
  ...
  const nickname = cleanText(payload?.nickname, 40);
  const pin = cleanText(payload?.pin, 12);
  if (!teacherId || !classId || !studentId || !seatNo || !nickname || pin.length < 4) return jsonError("編輯學生時請一併設定至少 4 碼的新 PIN。");

  const db = await ensureDb();
  if (!(await ownedActiveClass(db, teacherId, classId))) return jsonError("無這個班級的管理權。", 403);
  try {
    const result = await db
      .prepare("UPDATE students SET seat_no = ?, nickname = ?, pin_hash = ? WHERE id = ? AND class_id = ?")
      .bind(seatNo, nickname, await hashPin(`${classId}:${seatNo}`, pin), studentId, classId)
      .run();
